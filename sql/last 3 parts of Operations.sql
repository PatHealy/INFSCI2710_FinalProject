...
select c.*
from (select cases.*, orders.*
	  from cases
	  inner join orders
	  on cases.product_id = orders.product_id) c,(select customer_id,count(*) as num 
												   from orders 
...
from (select cases.*, orders.*
	  from cases
	  inner join orders
	  on cases.product_id = orders.product_id) c,(select cu.customer_id,cu.c_company 
